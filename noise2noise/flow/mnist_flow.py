...
class MNISTFashionFlow(Dataset):
    ...
    def _create_image(self, valid_classes, n_range):
...
#        break_num = self.img_n_insets*10
#        n_iter = 0
#        while labels2add and n_iter < break_num:
#            xr = random.randint(0, self.top_lim_x)
#            yr = random.randint(0, self.top_lim_y)
#            prev = img_o[xr:xr + self.in_size_x, yr:yr + self.in_size_y]
#            if not np.any(prev):
#                lab_i = labels2add.pop()
#                prev[:] = random.choice(inset_classes[lab_i])
#            
#            n_iter += 1
        return img_o
...
if __name__ == '__main__':
    ...
    gen = MNISTFashionFlow(output_size = 256,
                         epoch_size = 10,
                         bg_n_range = (5, 25),
                         int_range = (1., 1.),
                         max_rotation = 45,
